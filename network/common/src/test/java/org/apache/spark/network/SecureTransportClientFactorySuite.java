...
public class SecureTransportClientFactorySuite {
  private TransportConf conf;
  private TransportContext context;
  private TransportServer sslServer1;
  private TransportServer sslServer2;
  ...
  public void setUp() {
    ...
    context = new TransportContext(
      conf, rpcHandler, new SslEncryptionHandler(SSLSampleConfigs.createTrustedSSLFactory(), conf));
    sslServer1 = context.createServer();
    sslServer2 = context.createServer();
  }

  @After
  public void tearDown() {
    JavaUtils.closeQuietly(sslServer1);
    JavaUtils.closeQuietly(sslServer2);
  }
  ...
  private void testClientReuse(final int maxConnections, boolean concurrent)
    ...
    final int serverPort = sslServer1.getPort();
    ...
    for (int i = 0; i < attempts.length; i++) {
      attempts[i] = new Thread() {
        ...
        public void run() {
          try {
            TransportClient client = factory.createClient(TestUtils.getLocalHost(), serverPort);
  ...
  public void returnDifferentClientsForDifferentServers() throws IOException {
    TransportClientFactory factory1 = context.createClientFactory();
    TransportClientFactory factory2 = context.createClientFactory();
    TransportClient c1 = factory1.createClient(TestUtils.getLocalHost(), sslServer1.getPort());
    TransportClient c2 = factory2.createClient(TestUtils.getLocalHost(), sslServer2.getPort());
  ...
  @Test
  public void neverReturnInactiveSecureClients() throws IOException, InterruptedException {
    TransportClientFactory factory1 = context.createClientFactory();
    TransportClientFactory factory2 = context.createClientFactory();
    TransportClient c1 = factory1.createClient(TestUtils.getLocalHost(), sslServer1.getPort());
    c1.close();
